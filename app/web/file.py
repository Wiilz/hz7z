...
from ..extensions.interface.user_interface import is_anonymous
...
def upload_img():
    current_app.logger.info(
        f">>>  {'anonymous' if is_anonymous else getattr(request, 'user').username} Uploading Files  <<<")
    check_file_size()
    ...
    folder = allowed_folder(data.get('type'))
    if not file:
        raise ParamsError(u'上传有误')
    file_data, video_thum, video_dur, upload_type = _upload_file(file, folder)
...
def batch_upload():
    current_app.logger.info(
        f">>>  {'anonymous' if is_anonymous else getattr(request, 'user').username} Bulk Uploading Files  <<<")
    check_file_size()
...
def allowed_folder(folder):
    return folder if folder in ['media', 'article', 'icon', 'banner', 'avatar'] else 'hz7z'
